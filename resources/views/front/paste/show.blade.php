...
<meta name="description" content="{{ $paste->title_f }}">
<meta name="keywords" content="{{ $paste->tags }}">
...
<meta name="twitter:url" value="{{ $paste->url }}">
<meta name="twitter:title" content="{{ $paste->title_f }}">
<meta name="twitter:description" content="{{ str_limit($paste->description,255) }}">
@if(!empty(config('settings.site_image')))
<meta name="twitter:card" content="summary_large_image">
<meta name="twitter:image" content="{{url(config('settings.site_image'))}}" />
@endif
@stop
		...
		<div class="container">
			...
			<div class="row ">
				<div class="@if(config('settings.site_layout') == 1) col-md-9 @else col-md-12 @endif">
					...
					<div class="card">
						<div class="card-body">
							<div class="media"> @if(isset($paste->user))
									<img class="mr-3 rounded-circle img-fluid" src="{{$paste->user->avatar}}" alt="avatar" style="height: 60px"> @else
									<img class="mr-3 rounded-circle img-fluid" src="{{url('img/default-avatar.png')}}" alt="avatar" style="height: 60px"> @endif
								...
								@if(Auth::check())
									@if($paste->user_id == Auth::user()->id)
										...
										<a href="{{route('paste.delete',[$paste->slug])}}" class="badge badge-danger" onclick="return confirm('{{ __('Are you sure that you want to delete this paste') }}?')"><i class="fa fa-trash"></i> {{ __('Delete')}}
										</a> @else
										@if(Auth::user()->role == 1)
											<a href="{{route('pastes.edit',[$paste->id])}}" class="badge badge-info mr-2"><i class="fa fa-edit"></i> {{ __('Edit')}}
											</a>
											<a href="{{route('pastes.delete',[$paste->id])}}" class="badge badge-danger" onclick="return confirm('{{ __('Are you sure that you want to delete this paste') }}?')"><i class="fa fa-trash"></i> {{ __('Delete')}}
											</a> @endif
									@endif
								@endif 
							</div>
							...
							<div class="card mt-2"  id="printarea">
								<div class="card-header d-print-none">
									<span class="badge badge-light">{{get_syntax_name($paste->syntax)}}</span>
							...
							<div class="card mt-2">
								...
								<div class="card-body">
									<p>{{ (!empty($paste->description)) ? $paste->description : __('No description') }}
									</p>
								</div>
								@if(!empty($paste->tags))
								<div class="card-footer">
									<strong>{{ __('Tags') }}:</strong> 
									@foreach(explode(',', $paste->tags) as $tag)
										<a href="{{ route('archive',[$paste->syntax, 'tag' => $tag]) }}"><div class="badge badge-info">{{ $tag }}</div></a>
									@endforeach
								</div>
								@endif
							</div>
					...
					@if(config('settings.comments') == 1){!!html_entity_decode(config('settings.comments_code'))!!}@endif

					@if(config('settings.facebook_comments') == 1)
                		<div class="fb-comments" data-href="{{ $paste->url }}" data-numposts="5" data-width="100%"></div>
              		@endif
				</div>
			...
			<div class="modal-dialog">
				<div class="modal-content">
					...
					<div class="modal-body text-center"> @if(config('settings.qr_code_share') == 1)
							<div class="mb-2">
								<img src="https://api.qrserver.com/v1/create-qr-code/?size=150x150&data={{urlencode($paste->url)}}">
							</div>
						@endif
