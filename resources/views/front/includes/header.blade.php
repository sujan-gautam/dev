			...
			<div class="collapse navbar-collapse" id="navbarSupportedContent">
				...
				<ul class="navbar-nav mr-right">
					<li class="nav-item btn-group">
						@if(!empty(get_selected_locale()))<a class="nav-link dropdown-toggle" id="navbarDropdownMenuLink" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
							<small class="flag-icon flag-icon-{{ get_selected_locale()->country_code }}"></small> {{ get_selected_locale()->name }}
						</a>
						@endif
						<div class="dropdown-menu dropdown-primary mw-0 small" aria-labelledby="navbarDropdownMenuLink" style="min-width: 5rem !important;font-size:0.8rem !important;"> 
							@forelse(get_locales() as $lang)
								<a class="dropdown-item p-1 text-center" href="{{url('lang/'.$lang->code)}}" style="font-size: 0.8rem;"><span class="flag-icon flag-icon-{{ $lang->country_code }}"></span> {{$lang->name}}</a> 
							@endforeach
						</div>
					</li>
					@if(Auth::check())
